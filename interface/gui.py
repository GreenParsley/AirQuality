...
from interface.rawchart_page import RawChartPage
...
def Fill():
    if extended:  # If the frame is exanded show text and remove image
        ...
        raw_chart_data.config(text='Raw chart', image='', font=(0, 20))
    else:
        # Show image again
        ...
        raw_chart_data.config(image=raw_chart, font=(0, 20))
...
raw_chart_data = Button(frame, image=raw_chart, bg='LightSkyBlue2', relief='flat', command=lambda: ShowPage(page5))
...
raw_chart_data.grid(row=5, column=0, pady=15)
